...
faceDetect = cv2.CascadeClassifier('haarcascade_frontalface_default.xml');  ###using openCV cascade
cam = cv2.VideoCapture(0);                                 ###camera
rec = cv2.face.LBPHFaceRecognizer_create()
rec.read("recognizer\\trainningData.yml")
id = 0

def  getProfile(id) :                                                 ##l#ink sqlite
...
while(True):
    ...
    for (x,y,w,h) in faces :
        ...
        if (conf < 50) : 
            ...
            cv2.putText(img,str("Unknown"),(x,y+30),cv2.FONT_HERSHEY_COMPLEX_SMALL,2,255)
